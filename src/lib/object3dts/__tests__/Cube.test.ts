...
test('Check params are well passed', () =>{
  const object = new Cube({width, height, depth}, [], ObjectsCommon.createViewOptions(), new Scene());
...
test('Cube - toJSON - ViewOptions', () => {
  ...
  const object = new Cube({width, height, depth},[],ObjectsCommon.createViewOptions(color,visible,highlighted,name), new Scene());
  const obj:ICubeJSON = object.toJSON() as ICubeJSON;
...
test('Cube - fromJSON - ViewOptions', () => {
  ...
  const object1 = new Cube({width, height, depth},[],ObjectsCommon.createViewOptions(color,visible,highlighted,name), new Scene());
  const json1:ICubeJSON = object1.toJSON() as ICubeJSON;
  const object = Cube.newFromJSON(json1,new Scene());
  const obj:ICubeJSON = object.toJSON() as ICubeJSON;
  expect(obj.viewOptions.color).toEqual(color);
...
test('Cube - clone() - Parameteres', () => {
  const aux = new Cube({width, height, depth}, [], ObjectsCommon.createViewOptions(), new Scene());
