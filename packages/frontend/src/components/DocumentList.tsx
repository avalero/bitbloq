...
const DocumentList: FC<IDocumentListProps> = ({
  ...
  const onDocumentRenameClick = (
    ...
    e.stopPropagation();
    setSelectedToMove({ id: "" });
    setEditDocNameModal({ id: document.id!, name: document.title });
  };
  ...
  const onFolderRenameClick = (
    ...
    setEditFolderNameModal({
      id: folder.id!,
      name: (folder as IDocument).title
    });
  };
  ...
  const onUpdateDocName = async (docName?: string) => {
    await updateDocument({
      variables: {
        ...
        title: docName ? docName : "Documento sin título"
      }
    });
    refetchDocsFols();
  ...
  const onDuplicateDocument = async (
    ...
    e.stopPropagation();
    let newName: string = `${document.title} copia`;
